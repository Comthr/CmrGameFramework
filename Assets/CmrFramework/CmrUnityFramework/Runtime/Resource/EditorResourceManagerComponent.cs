...
using CmrGameFramework.Download;
...
namespace CmrUnityGameFramework.Runtime
{
    ...
    public sealed class EditorResourceManagerComponent : MonoBehaviour, IResourceManager
    {
        ...
        private CachedLinkList<LoadAssetInfo> m_LoadAssetInfos = null;
        private CachedLinkList<LoadSceneInfo> m_LoadSceneInfos = null;
        private CachedLinkList<UnloadSceneInfo> m_UnloadSceneInfos = null;
...
#pragma warning disable 0067, 0414
        ...
        /// 资源校验开始事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceVerifyStartEventArgs> ResourceVerifyStart = null;

        /// <summary>
        /// 资源校验成功事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceVerifySuccessEventArgs> ResourceVerifySuccess = null;

        /// <summary>
        /// 资源校验失败事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceVerifyFailureEventArgs> ResourceVerifyFailure = null;

        /// <summary>
        /// 资源应用开始事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceApplyStartEventArgs> ResourceApplyStart = null;

        /// <summary>
        /// 资源应用成功事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceApplySuccessEventArgs> ResourceApplySuccess = null;

        /// <summary>
        /// 资源应用失败事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceApplyFailureEventArgs> ResourceApplyFailure = null;

        /// <summary>
        /// 资源更新开始事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceUpdateStartEventArgs> ResourceUpdateStart = null;

        /// <summary>
        /// 资源更新改变事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceUpdateChangedEventArgs> ResourceUpdateChanged = null;

        /// <summary>
        /// 资源更新成功事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceUpdateSuccessEventArgs> ResourceUpdateSuccess = null;

        /// <summary>
        /// 资源更新失败事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceUpdateFailureEventArgs> ResourceUpdateFailure = null;

        /// <summary>
        /// 资源更新全部完成事件。
        /// </summary>
        public event EventHandler<CmrGameFramework.Resource.ResourceUpdateAllCompleteEventArgs> ResourceUpdateAllComplete = null;
...
#pragma warning restore 0067, 0414
        ...
        private void Awake()
        {
            /*
             * CYJStep0-4:
             * 这个类的作用一目了然，继承自IResourceManager，说明它是满足作为一个资源模块的要求的。
             * 当然很多接口方法的事件只是抛出一个'不支持'的异常。
             * 但在如果在BaseComponent中勾选了 EditorResourceMode，则其他模块在调用资源模块的时候，会优先使用这个组件
             * 所以它是比较特殊的。内部实现也很简单，对数据做了初始化，包括BaseComponent的EditorResourceMode进行赋值，给其他模块提供资源模块功能
             * 接下来就是去对每一个BaseComponent下的组件类的Awake方法分别进行功能分析。
             * 但很多组件的Awake方法的职能是重复的，因此我只会挑几个特例来说，功能重复的部分就掠过
             * 按照顺序，如果没有额外设置的话，按照场景Hierarchy顺序，第一个调用Awake的就是Config模块了
             * 移步至ConfigComponent的Awake方法吧→
             */
            m_ReadOnlyPath = null;
            m_ReadWritePath = null;
            m_CachedAssets = new Dictionary<string, UnityEngine.Object>(StringComparer.Ordinal);
            m_LoadAssetInfos = new CachedLinkList<LoadAssetInfo>();
            m_LoadSceneInfos = new CachedLinkList<LoadSceneInfo>();
            m_UnloadSceneInfos = new CachedLinkList<UnloadSceneInfo>();
            ...
            if (m_LoadAssetInfos.Count > 0)
            {
                ...
                while (current != null && count < m_LoadAssetCountPerFrame)
                {
                    ...
                    if (elapseSeconds >= loadAssetInfo.DelaySeconds)
                    {
                        ...
                        if (asset != null && loadAssetInfo.LoadAssetCallbacks.LoadAssetSuccessCallback != null)
                        ...
                        else if (loadAssetInfo.LoadAssetCallbacks.LoadAssetFailureCallback != null)
        ...
        public void LoadAsset(string assetName, Type assetType, int priority, LoadAssetCallbacks loadAssetCallbacks, object userData)
        {
            ...
            //CYJ:加入待加载资源中，会在Update函数中处理
            m_LoadAssetInfos.AddLast(new LoadAssetInfo(assetName, assetType, priority, DateTime.UtcNow,
                m_MinLoadAssetRandomDelaySeconds + (float)Utility.Random.GetRandomDouble() * (m_MaxLoadAssetRandomDelaySeconds - m_MinLoadAssetRandomDelaySeconds),
