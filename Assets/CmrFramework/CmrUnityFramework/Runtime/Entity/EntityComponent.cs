...
namespace CmrUnityGameFramework.Runtime
{
    ...
    [AddComponentMenu("CmrFramework/Entity")]
    public sealed partial class EntityComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
             ...
             * CYJStep1-5:
             * 当这里位置都还算可以理解。
             * 下面的if方法体中也只是给实体根节点做了实例化，
             * 然后就是遍历了所有EntityGroup，将他们AddEntityGroup
             * 这里的m_EntityGroups是可以在组件的Inspector面板中添加的。
             * 接着就将Group加入模块层面的字典中。并且创建一个框架层的EntityGroup实例。
             * 在Runtime层的EntityGroup仅包含一些信息，提供给创建框架层的EntityGroup的初始化信息
             * 框架层的EntityGroup维护一个对象池，具体的更多细节就在后续再说吧，
             * 现在就先移步到最关键的部分ProcedureComponent→
             */
            ...
            for (int i = 0; i < m_EntityGroups.Length; i++)
            {
                ...
                //    continue;
                //}
                //CYJALTET:这里这么一长串也太吓人了，而且会给m_EntityGroups在IDE中加很多引用，不方便阅读
                EntityGroup group = m_EntityGroups[i];
                bool isSuccessful = AddEntityGroup(group.Name, group.InstanceAutoReleaseInterval, 
                    group.InstanceCapacity, group.InstanceExpireTime, group.InstancePriority);
                if (!isSuccessful)
                {
        ...
        public void AttachEntity(Entity childEntity, Entity parentEntity, string parentTransformPath, object userData)
        {
            ...
            else
            {
                ...
                {
                    Log.Warning($"Can not find transform path '{parentTransformPath}' from parent entity '{parentEntity.Logic.Name}'.");
                    parentTransform = parentEntity.Logic.CachedTransform;
                }
            }
        ...
        private void OnShowEntitySuccess(object sender, CmrGameFramework.Entity.ShowEntitySuccessEventArgs e)
        {
            m_EventComponent.Fire(this, ShowEntitySuccessEventArgs.Create(e));
        }

        private void OnShowEntityFailure(object sender, CmrGameFramework.Entity.ShowEntityFailureEventArgs e)
        {
            Log.Warning($"Show entity failure, entity id '{e.EntityId}', asset name '{e.EntityAssetName}', entity group name '{e.EntityGroupName}', error message '{e.ErrorMessage}'.");
            m_EventComponent.Fire(this, ShowEntityFailureEventArgs.Create(e));
        }

        private void OnShowEntityUpdate(object sender, CmrGameFramework.Entity.ShowEntityUpdateEventArgs e)
        {
            m_EventComponent.Fire(this, ShowEntityUpdateEventArgs.Create(e));
        }

        private void OnShowEntityDependencyAsset(object sender, CmrGameFramework.Entity.ShowEntityDependencyAssetEventArgs e)
        {
            m_EventComponent.Fire(this, ShowEntityDependencyAssetEventArgs.Create(e));
        }

        private void OnHideEntityComplete(object sender, CmrGameFramework.Entity.HideEntityCompleteEventArgs e)
        {
