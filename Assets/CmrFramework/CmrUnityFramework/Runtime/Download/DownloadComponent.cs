...
namespace CmrUnityGameFramework.Runtime
{
    ...
    [AddComponentMenu("CmrFramework/Download")]
    public sealed class DownloadComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
            ...
            /*
             * CYJStep1-2:
             * 这里不同于其他地方，会根据设定好的下载代理个数，创建对应个数的helper。
             * 这里值得进去看看→
             */
        ...
        private void AddDownloadAgentHelper(int index)
        {
            ...
            /*
             * CYJStep1-3:
             * 这里的逻辑大体差不多，就是创建helper，并放置在场景中。
             * 但其他组件可能就只是向模块对应的helper字段赋值上当前创建的引用，
             * 但这里做的事情要更多一些，进入AddDownloadAgentHelper方法→(通过搜索DownloadManager再搜索方法AddDowanloadAgentHelper)
             */
        ...
        private void OnDownloadStart(object sender, CmrGameFramework.Download.DownloadStartEventArgs e)
        {
            m_EventComponent.Fire(this, DownloadStartEventArgs.Create(e));
        }

        private void OnDownloadUpdate(object sender, CmrGameFramework.Download.DownloadUpdateEventArgs e)
        {
            m_EventComponent.Fire(this, DownloadUpdateEventArgs.Create(e));
        }

        private void OnDownloadSuccess(object sender, CmrGameFramework.Download.DownloadSuccessEventArgs e)
        {
            m_EventComponent.Fire(this, DownloadSuccessEventArgs.Create(e));
        }

        private void OnDownloadFailure(object sender, CmrGameFramework.Download.DownloadFailureEventArgs e)
        {
            Log.Warning($"Download failure, download serial id '{e.SerialId}', download path '{e.DownloadPath}', download uri '{e.DownloadUri}', error message '{e.ErrorMessage}'.");
