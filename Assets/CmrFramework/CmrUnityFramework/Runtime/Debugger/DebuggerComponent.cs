...
namespace CmrUnityFramework.Runtime
{
    ...
    [AddComponentMenu("CmrFramework/Debugger")]
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        ...
        private void Start()
        {
            /*
             * CYJStep1-1:
             * 在这里，调试窗口会被全部注册。注册的具体逻辑就是对路径进行分割，父级注册成WindowGroup
             * 注册的是(名字,引用)
             * 然后设置好界面的显示标识，就算完了。
             * 后续会提到他是如何把面板绘制到游戏界面上的，但现在先去到下一步吧。DownloadComponent→
             */
        ...
        private void DrawDebuggerWindowIcon(int windowId)
        {
            ...
            string title = $"<color=#{color.r:x2}{color.g:x2}{color.b:x2}{color.a:x2}><b>FPS: {m_FpsCounter.CurrentFps:F2}</b></color>";
