...
using CmrGameFramework.Debugger;
using System;
    ...
    public sealed partial class DebuggerComponent : GameFrameworkComponent
    {
        ...
        private sealed class ConsoleWindow : IDebuggerWindow
        {
                ...
                GUILayout.BeginVertical("box");
                {
                    m_StackScrollPosition = GUILayout.BeginScrollView(m_StackScrollPosition, GUILayout.Height(100f));
                    {
                        if (m_SelectedNode != null)
                        {
                            ...
                            if (GUILayout.Button($"<color=#{color.r:x2}{color.g:x2}{color.b:x2}{color.a:x2}><b>{m_SelectedNode.LogMessage}</b></color>{Environment.NewLine}{Environment.NewLine}{m_SelectedNode.StackTrack}", "label"))
                            {
                                CopyToClipboard($"{m_SelectedNode.LogMessage}{Environment.NewLine}{Environment.NewLine}{m_SelectedNode.StackTrack}");
                            }
                        }
                    }
                    GUILayout.EndScrollView();
            ...
            private string GetLogString(LogNode logNode)
            {
                ...
                return $"<color=#{color.r:x2}{color.g:x2}{color.b:x2}{color.a:x2}>[{logNode.LogTime.ToLocalTime():HH:mm:ss.fff}][{logNode.LogFrameCount}] {logNode.LogMessage}</color>";
            }
