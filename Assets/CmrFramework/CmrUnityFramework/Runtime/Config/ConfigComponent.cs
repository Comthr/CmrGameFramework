...
namespace CmrUnityGameFramework.Runtime
{
    ...
    [AddComponentMenu("CmrFramework/Config")]
    public sealed class ConfigComponent : GameFrameworkComponent
    {
        ...
        [SerializeField]
        private string m_ConfigHelperTypeName = "CmrFramework.Runtime.DefaultConfigHelper";
        ...
        protected override void Awake()
        {
            ...
            /*
             * CYJStep0-5:
             * 关键点来了。这里会以IConfigManager为泛型参数调用GetMoule方法，我们不妨进去看看具体是什么?→
             */
            ...
            /*
             * CYJStep0-5-3:
             * 接下来的就比较简单了，就是对给事件添加向事件模块进行广播的方法监听
             * 大部分的模块在Awake中就是在干这些事情，获取模块、向事件模块注册
             * 节省时间，一部分模块就不一一介绍过去了，只挑有额外功能的就说，忽略的组件部分可能涉及到一些字段的初始化，但也都无关紧要
             * 所以在Awake中，我们就直接去移步到FileSystemComponent的Awake吧→
             */
        ...
        private void Start()
        {
            /*
             * CYJStep1:
             * 粗略说一下这里吧，这里做的事情大体和Awake职责差不多。
             * 因为需要获取到事件模块，因此就需要放到Start中等待事件组件被注册。
             * 此外就是一些Helper的创建和设置
             * 没问题，移步DebuggerComponent中的Start方法
             */
