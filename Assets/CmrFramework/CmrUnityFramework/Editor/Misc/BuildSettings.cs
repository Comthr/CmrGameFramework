...
namespace CmrUnityGameFramework.Editor
{
    /// <summary>
    /// 构建配置相关的实用函数。
    /// CYJTodo:当前逻辑是读取对应特性字段的路径xml，后续可以改成自动生成。
    /// </summary>
    internal static class BuildSettings
    {
        ...
        {
            s_ConfigurationPath = Type.GetConfigurationPath<BuildSettingsConfigPathAttribute>() ?? Utility.Path.GetRegularPath(Path.Combine(Application.dataPath, "CmrGameFramework/Configs/BuildSettings.xml"));
        ...
        /// 将构建场景设置为默认。
        /// </summary>
        [MenuItem("CmrFramework/Scenes in Build Settings/Default Scenes", false, 20)]
        ...
        /// 将构建场景设置为所有。
        /// </summary>
        [MenuItem("CmrFramework/Scenes in Build Settings/All Scenes", false, 21)]
