...
namespace CmrUnityGameFramework.Editor
{
    ...
    internal sealed class DownloadComponentInspector : GameFrameworkInspector
    {
                        ...
                        if (GUILayout.Button("Export CSV Data"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(exportFileName))
                            {
                                try
                                {
                                    ...
                                    foreach (TaskInfo downloadInfo in downloadInfos)
                                    {
                                        data[index++] = $"{downloadInfo.Description},{downloadInfo.SerialId},{downloadInfo.Tag ?? string.Empty},{downloadInfo.Priority},{downloadInfo.Status}";
                                    }
        ...
        {
            EditorGUILayout.LabelField(downloadInfo.Description, $"[SerialId]{downloadInfo.SerialId} [Tag]{downloadInfo.Tag ?? "<None>"} [Priority]{downloadInfo.Priority} [Status]{downloadInfo.Status}");
        }
