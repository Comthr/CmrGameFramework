...
namespace CmrGameFramework.FileSystem
{
    /// <summary>
    /// 文件系统。
    /// </summary>
    internal sealed partial class FileSystem : IFileSystem
    {
        ...
        private FileSystem(string fullPath, FileSystemAccess access, FileSystemStream stream)
        {
            ...
            m_FreeBlockIndexes = new MultiDictionary<int, int>();
        ...
        private bool TryCombineFreeBlocks(int freeBlockIndex)
        {
            ...
            foreach (KeyValuePair<int, LinkedListRange<int>> blockIndexes in m_FreeBlockIndexes)
            {
        ...
        {
            LinkedListRange<int> lengthRange = default(LinkedListRange<int>);
        ...
        private int AllocBlock(int length)
        {
            ...
            int lengthFound = -1;
            LinkedListRange<int> lengthRange = default;
            foreach (KeyValuePair<int, LinkedListRange<int>> i in m_FreeBlockIndexes)
            {
                if (i.Key < length)
                {
                    continue;
