...
namespace CmrGameFramework.Entity
{
    /// <summary>
    /// 实体管理器。
    /// </summary>
    internal sealed partial class EntityManager : GameFrameworkModule, IEntityManager
    {
        ...
        public bool HasEntityGroup(string entityGroupName)
        {
            if (string.IsNullOrEmpty(entityGroupName))
                throw Utility.Exception.ThrowException("Entity group name is invalid.");
        ...
        public bool AddEntityGroup(string entityGroupName, float instanceAutoReleaseInterval, int instanceCapacity, float instanceExpireTime, int instancePriority, IEntityGroupHelper entityGroupHelper)
        {
            ...
            if (entityGroupHelper == null)
                throw Utility.Exception.ThrowException("Entity group helper is invalid.");

            if (m_ObjectPoolManager == null)
                throw Utility.Exception.ThrowException("You must set object pool manager first.");
        ...
        public void ShowEntity(int entityId, string entityAssetName, string entityGroupName, int priority, object userData)
        {
            ...
            if (m_EntityHelper == null)
                throw Utility.Exception.ThrowException("You must set entity helper first.");

            if (string.IsNullOrEmpty(entityAssetName))
                throw Utility.Exception.ThrowException("Entity asset name is invalid.");

            if (string.IsNullOrEmpty(entityGroupName))
                throw Utility.Exception.ThrowException("Entity group name is invalid.");

            if (HasEntity(entityId))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Entity id '{0}' is already exist.", entityId));

            if (IsLoadingEntity(entityId))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Entity '{0}' is already being loaded.", entityId));

            EntityGroup entityGroup = (EntityGroup)GetEntityGroup(entityGroupName);
            if (entityGroup == null)
                throw Utility.Exception.ThrowException(Utility.Text.Format("Entity group '{0}' is not exist.", entityGroupName));
        ...
        private void InternalShowEntity(int entityId, string entityAssetName, EntityGroup entityGroup, object entityInstance, bool isNewInstance, float duration, object userData)
        {
            try
            {
                ...
                if (entity == null)
                    throw Utility.Exception.ThrowException("Can not create entity in entity helper.");
        ...
        private void LoadAssetSuccessCallback(string entityAssetName, object entityAsset, float duration, object userData)
        {
            ...
            if (showEntityInfo == null)
                throw Utility.Exception.ThrowException("Show entity info is invalid.");
        ...
        private void LoadAssetFailureCallback(string entityAssetName, LoadResourceStatus status, string errorMessage, object userData)
        {
            ...
            m_EntitiesBeingLoaded.Remove(showEntityInfo.EntityId);
            string appendErrorMessage = $"Load entity failure, asset name '{entityAssetName}', status '{status}', error message '{errorMessage}'.";
            if (m_ShowEntityFailureEventHandler != null)
            {
