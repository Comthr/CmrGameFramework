...
namespace CmrGameFramework.ObjectPool
{
    ...
    internal sealed partial class ObjectPoolManager : GameFrameworkModule, IObjectPoolManager
    {
        ...
        {
            if (objectType == null)
                throw Utility.Exception.ThrowException("Object type is invalid.");

            if (!typeof(ObjectBase).IsAssignableFrom(objectType))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Object type '{0}' is invalid.", objectType.FullName));
        ...
        {
            if (condition == null)
                throw Utility.Exception.ThrowException("Condition is invalid.");
        ...
        {
            if (objectPool == null)
                throw Utility.Exception.ThrowException("Object pool is invalid.");
        ...
        private IObjectPool<T> InternalCreateObjectPool<T>(string name, bool allowMultiSpawn, float autoReleaseInterval, int capacity, float expireTime, int priority) where T : ObjectBase
        {
            ...
            if (HasObjectPool<T>(name))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Already exist object pool '{0}'.", typeNamePair));
