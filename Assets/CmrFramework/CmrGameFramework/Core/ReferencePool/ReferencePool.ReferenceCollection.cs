        ...
        private class ReferenceCollection
        {
            ...
            public T Acquire<T>() where T : class, IReference, new()
            {
                if ((object)typeof(T) != m_ReferenceType)
                    throw Utility.Exception.ThrowException("Type is invalid.");
                m_UsingReferenceCount++; m_AcquireReferenceCount++;
            ...
            public void Release(IReference reference)
            {
                ...
                lock (m_References)
                {
                    if (m_EnableStrictCheck && m_References.Contains(reference))
                        throw Utility.Exception.ThrowException("The reference has been released."); m_References.Enqueue(reference);
                }
            ...
            public void Add<T>(int count) where T : class, IReference, new()
            {
                ...
                lock (m_References)
                {
                    m_AddReferenceCount += count; while (count-- > 0) m_References.Enqueue(new T());
                }
            }
            ...
            public void Remove(int count)
            {
                lock (m_References)
                {
                    if (count > m_References.Count)
                        count = m_References.Count; m_RemoveReferenceCount += count;
                    while (count-- > 0)
