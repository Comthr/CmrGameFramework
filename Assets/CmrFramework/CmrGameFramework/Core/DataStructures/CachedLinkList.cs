...
namespace CmrGameFramework
{
    /// <summary>
    /// 缓存链表。
    /// CYJ:省流总结。内部就是个队列+链表，每次删除节点时就将其存到缓存队列中，
    /// 每次获得链表的时候就优先使用缓存中的对象，没有就再new。
    /// </summary>
    /// <typeparam name="T">指定链表的元素类型。</typeparam>
    public sealed class CachedLinkList<T> : ICollection<T>, IEnumerable<T>, ICollection, IEnumerable
    {
        private readonly LinkedList<T> m_LinkedList;
        private readonly Queue<LinkedListNode<T>> m_CachedNodes;
        int? cacheCapacity = null;

        /// <param name="cacheCapacity">最大缓存容量，Nullable</param>
        public CachedLinkList(int? cacheCapacity = null)
        {
        ...
        /// <summary>
        /// CYJ: 获得链表,先从缓存拿对应对象，没有就new一个
        /// </summary>
        /// <param name="value">节点值</param>
        /// <returns>获得的节点</returns>
        ...
        {
            node.Value = default;
            if(cacheCapacity==null|| m_CachedNodes.Count < cacheCapacity)
