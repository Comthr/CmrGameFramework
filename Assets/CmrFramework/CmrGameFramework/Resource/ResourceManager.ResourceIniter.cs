...
namespace CmrGameFramework.Resource
{
    internal sealed partial class ResourceManager : GameFrameworkModule, IResourceManager
    {
        ...
        private sealed class ResourceIniter
        {
            ...
            /// <summary>
            /// CYJ:加载GameFrameworkVersion.dat文件成功回调，
            /// GameFrameworkVersion反序列化后就是一个PackageVersionList类
            /// </summary>
            private void OnLoadPackageVersionListSuccess(string fileUri, byte[] bytes, float duration, object userData)
            {
                ...
                try
                {
                    ...
                    if (!versionList.IsValid)
                        throw Utility.Exception.ThrowException("Deserialize package version list failure.");
                    ...
                    ResourceInitComplete();//CYJ:在这里调用了资源初始化的回调。
                }
                catch (Exception exception)
                {
                    ...
                    throw Utility.Exception.ThrowException(Utility.Text.Format("Parse package version list exception '{0}'.", exception), exception);
                }
                finally
                {
            ...
            {
                throw Utility.Exception.ThrowException(Utility.Text.Format("Package version list '{0}' is invalid, error message is '{1}'.", fileUri, string.IsNullOrEmpty(errorMessage) ? "<Empty>" : errorMessage));
            }
        }
    }
}
