...
namespace CmrGameFramework.Debugger
{
    internal sealed partial class DebuggerManager : GameFrameworkModule, IDebuggerManager
    {
        ...
        private sealed class DebuggerWindowGroup : IDebuggerWindowGroup
        {
            ...
            {
                if (string.IsNullOrEmpty(path))
                    throw Utility.Exception.ThrowException("Path is invalid.");

                int pos = path.IndexOf('/');
                //CYJ:若不存在'/'说明就是根节点直接是一个window，直接注册

                if (pos < 0 || pos >= path.Length - 1)
                {
                    if (InternalGetDebuggerWindow(path) != null)
                        throw Utility.Exception.ThrowException("Debugger window has been registered.");
                ...
                else
                {
                    ...
                    //CYJConfuse:这里不怕类型转换导致报错吗？如果有人不小心用了一个注册过的非Group面板，就异常了
                    //CYJALTER:改成了as
                    //Original:
                    //DebuggerWindowGroup debuggerWindowGroup = (DebuggerWindowGroup)InternalGetDebuggerWindow(debuggerWindowGroupName);
                    IDebuggerWindow window = InternalGetDebuggerWindow(debuggerWindowGroupName);
                    DebuggerWindowGroup debuggerWindowGroup = window as DebuggerWindowGroup;
                    if (debuggerWindowGroup == null)
                    {
                        if (window != null)
                            throw Utility.Exception.ThrowException("Debugger window has been registered, can not create debugger window group.");
