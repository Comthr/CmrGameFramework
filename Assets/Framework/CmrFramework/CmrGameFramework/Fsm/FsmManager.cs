...
{
    internal class FsmManager: GameFrameworkModule, IFsmManager
    {
        private readonly Dictionary<TypeNamePair, FsmBase> m_Fsms;

        /// <summary> 
        /// CYJ:轮询前清空，轮询时复制m_Fsms.Values并保存其中
        /// 防止轮询逻辑中存在DestroyFsm逻辑导致迭代报错中断 
        /// </summary>
        ...
        {
            if (ownerType == null)
                throw Utility.Exception.ThrowException("Owner type is invalid.");
        ...
        public IFsm<T> CreateFsm<T>(string name, T owner, params FsmState<T>[] states) where T : class
        {
            ...
            if (HasFsm<T>(name))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Already exist FSM '{0}'.", typeNamePair));
        ...
        {
            if (fsm == null)
                throw Utility.Exception.ThrowException("FSM is invalid.");
