...
namespace CmrGameFramework.Download
{
    /// <summary>
    /// 下载管理器。
    /// </summary>
    internal sealed partial class DownloadManager : GameFrameworkModule, IDownloadManager
    {
        ...
        public void AddDownloadAgentHelper(IDownloadAgentHelper downloadAgentHelper)
        {
            /*
             * CYJStep1-4:
             * 我们已经很深入了，这里已经是GameFramework框架的内部实现细节，
             * 之前都只是在UnityGameFramework中。这两者有着层级的关系。
             * 所以这里需要对拥有具体下载细节的helper做一个封装类，就是一个下载代理，
             * 给下载开始、轮询，成功失败事件都注册好对应方法。这些方法就是DownloadManager去触发对应的事件
             * 这样一个下载代理就做好了。接着，代理被加入到任务池中。
             * 任务池说白了就是管理某种任务代理的一个数据结构。在这里是下载代理池。
             * 做个比喻，agent就像外卖员，一个Task就像一份外卖，而TaskPool就像外卖平台。
             * 在平台TaskPool收到一个外卖派送任务Task时，就会调度空闲的骑手agent，分配它去执行这个task。
             * 但是如果骑手不够多的话，一份外卖就需要等待一个外卖员送完当前外卖后，再根据优先级送下一个。
             * 因此还需要管理正在等待的任务根据优先级分配给agent。
             * 这就是taskPool的功能，TaskPool.AddAgent会调用agent的Initialize，但这就超出我们的范畴了。
             * TaskPool和DowanloadAgent的细节我们在后续再说。现在就先在这里中止吧。
             * 我们去看下一个模块的Start方法，EntityComponent→
             */
