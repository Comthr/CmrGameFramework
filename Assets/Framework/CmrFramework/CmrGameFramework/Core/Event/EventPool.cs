...
namespace CmrGameFramework
{
    ...
    internal sealed partial class EventPool<T> where T : BaseEventArgs
    {
        ...
        public bool Check(int id, EventHandler<T> handler)
        {
            if (handler == null)
            {
                throw Utility.Exception.ThrowException("Event handler is invalid.");
            }
        ...
        public void Subscribe(int id, EventHandler<T> handler)
        {
            ...
            {
                throw Utility.Exception.ThrowException(Utility.Text.Format("Event '{0}' not allow multi handler.", id));
            }
            else if ((m_EventPoolMode & EventPoolMode.AllowDuplicateHandler) != EventPoolMode.AllowDuplicateHandler && Check(id, handler))
            {
                throw Utility.Exception.ThrowException(Utility.Text.Format("Event '{0}' not allow duplicate handler.", id));
            }
            else
            {
                m_EventHandlers.Add(id, handler);
        ...
        public void Unsubscribe(int id, EventHandler<T> handler)
        {
            ...
            if (!m_EventHandlers.Remove(id, handler))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Event '{0}' not exists specified handler.", id));
        }
        ...
        {
            if (e == null)
                throw Utility.Exception.ThrowException("Event is invalid.");
        ...
        private void HandleEvent(object sender, T e)
        {
            ...
            LinkedListRange<EventHandler<T>> range = default;
            ...
            if (noHandlerException)
                throw Utility.Exception.ThrowException(Utility.Text.Format("Event '{0}' not allow no handler.", e.Id));
        }
    }
}