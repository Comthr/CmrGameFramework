    ...
    public abstract class GameFrameworkSerializer<T>
    {
        ...
        public bool Serialize(Stream stream, T data)
        {
            if (m_SerializeCallbacks.Count <= 0)
                throw Utility.Exception.ThrowException("No serialize callback registered.");
        ...
        public T Deserialize(Stream stream)
        {
            ...
            if (header0 != header[0] || header1 != header[1] || header2 != header[2])
                throw Utility.Exception.ThrowException($"Header is invalid, need '{(char)header[0]}{(char)header[1]}{(char)header[2]}', current '{(char)header0}{(char)header1}{(char)header2}'.");

            byte version = (byte)stream.ReadByte();
            DeserializeCallback callback = null;
            if (!m_DeserializeCallbacks.TryGetValue(version, out callback))
                throw Utility.Exception.ThrowException(Utility.Text.Format("Deserialize callback '{0}' is not exist.", version));
