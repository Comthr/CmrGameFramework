...
namespace CmrGameFramework
{
    ...
    internal sealed class DataProvider<T> : IDataProvider<T>
    {
        ...
        {
            if (ensureSize < 0)
                throw Utility.Exception.ThrowException("Ensure size is invalid.");
        ...
        public void ReadData(string dataAssetName, int priority, object userData)
        {
            if (m_ResourceManager == null)
                throw Utility.Exception.ThrowException("You must set resource manager first.");

            if (m_DataProviderHelper == null)
                throw Utility.Exception.ThrowException("You must set data provider helper first.");
            ...
            switch (result)
            {
                ...
                case HasAssetResult.BinaryOnFileSystem:
                    ...
                    if (dataLength != m_ResourceManager.LoadBinaryFromFileSystem(dataAssetName, s_CachedBytes))
                        throw Utility.Exception.ThrowException(Utility.Text.Format("Load binary '{0}' from file system with internal error.", dataAssetName));

                    try
                    {
                        if (!m_DataProviderHelper.ReadData(m_Owner, dataAssetName, s_CachedBytes, 0, dataLength, userData))
                            throw Utility.Exception.ThrowException(Utility.Text.Format("Load data failure in data provider helper, data asset name '{0}'.", dataAssetName));
                ...
                default:
                    throw Utility.Exception.ThrowException(Utility.Text.Format("Data asset '{0}' is '{1}'.", dataAssetName, result));
            }
        }
        ...
        {
            if (m_DataProviderHelper == null)
                throw Utility.Exception.ThrowException("You must set data helper first.");

            if (dataString == null)
                throw Utility.Exception.ThrowException("Data string is invalid.");
            ...
            catch (Exception exception)
            {
                ...
                throw Utility.Exception.ThrowException(Utility.Text.Format("Can not parse data string with exception '{0}'.", exception), exception);
            }
        }
        ...
        public bool ParseData(byte[] dataBytes, int startIndex, int length, object userData)
        {
            ...
            if (dataBytes == null)
                throw Utility.Exception.ThrowException("Data bytes is invalid.");

            if (startIndex < 0 || length < 0 || startIndex + length > dataBytes.Length)
                throw Utility.Exception.ThrowException("Start index or length is invalid.");
            ...
            catch (Exception exception)
            {
                if (exception is GameFrameworkException)
                    throw;
                throw Utility.Exception.ThrowException(Utility.Text.Format("Can not parse data bytes with exception '{0}'.", exception), exception);
            }
        }
        ...
        internal void SetResourceManager(IResourceManager resourceManager)
        {
            if (resourceManager == null)
                throw Utility.Exception.ThrowException("Resource manager is invalid.");
        ...
        internal void SetDataProviderHelper(IDataProviderHelper<T> dataProviderHelper)
        {
            if (dataProviderHelper == null)
                throw Utility.Exception.ThrowException("Data provider helper is invalid.");
        ...
        private void LoadAssetSuccessCallback(string dataAssetName, object dataAsset, float duration, object userData)
        {
            try
            {
                //m_DataProviderHelper是DefaultConfigHelper
        ...
        {
            string appendErrorMessage = $"Load data failure, data asset name '{dataAssetName}', status '{status}', error message '{errorMessage}'.";
            if (m_ReadDataFailureEventHandler != null)
            {
